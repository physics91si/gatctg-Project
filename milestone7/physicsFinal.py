...
def main():
    ...
    elif len(args) == 4 and args[2] == "pubmed" and args[3] == "wordcloud": #Make wordcloud from keywords searched from Pubmed
        wordCloudGenerator(concatString(runPubMed(args[0].lower(), args[1].lower(), '+synthetic+biology')))
        updateHTML(imgurUpload())
    else:
        print ("Something went wrong with your input. Please type your query in the form \"physicsFinal.py firstName last\
Name\"")
        print("If you'd like to search PubMed, type \"pubmed\" as a third argument")
        print("If you want to make a wordCloud using the searches above, just add \"wordcloud\" to the end of the searches above.")
        print("If you want to make a tweet from a wordcloud, use the format \"physicsFinal.py twitterhandle tweet\"")
...
def tweetCloud(username):
    '''This function actually coordinates the creation of the wordcloud and uploading to imgur'''
    wordCloudGenerator(getTweetKeywords(username))
    updateHTML(imgurUpload())
def concatString(array):
    '''Takes an array of strings and makes them into a single string with which to make the word cloud'''
    text = ""
    for words in array:
        text += " "
        text += words
